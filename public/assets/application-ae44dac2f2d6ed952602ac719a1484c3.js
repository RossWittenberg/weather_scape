...
function wind(breezes) {
	var windDiv = $('.wind');
	windDiv.empty();
	var skyWidth = $(window).width()
	var skyHeight = $(window).height()/2

	for( i=1;i<breezes;i++) {
		var breezeLeft = randRange(-2000,skyWidth);
		var breezeTop = randRange(0,skyHeight);
		$('<div>').addClass('windBreeze')
							.attr('id', 'windBreeze'+i)
							.appendTo(windDiv);
		$('#windBreeze'+i).css('left',breezeLeft);
		$('#windBreeze'+i).css('top',breezeTop);
	}
}
